...
exports.handler = async function(event, context) {
  try{
    ...
    try{ await redis.hincrby('worker:metrics', 'cron_run_count', 1); }catch(e){ console.warn('process_jobs_cron: could not increment cron_run_count', e?.message || e); }
    ...
    if(!len || Number(len) === 0){
      try{ await redis.hincrby('worker:metrics', 'cron_run_skipped', 1); }catch(e){ console.warn('process_jobs_cron: failed to increment cron_run_skipped', e?.message || e); }
    ...
    try{ await redis.hincrby('worker:metrics', 'cron_run_processed', 1); }catch(e){ console.warn('process_jobs_cron: failed to increment cron_run_processed', e?.message || e); }
    return res;
  }catch(err){
