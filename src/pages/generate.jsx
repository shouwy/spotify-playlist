...
function hrefToSpotifyUri(href){
  ...
  try{
    ...
    console.warn('Invalid href URL:', e);
    return null;
  }
}
...
export default function GeneratePage() {
  ...
  async function pollJobUntilDone(jobId, initialDelay) {
  ...
  async function generate() {
    ...
    try{
      ...
      try{
        ...
        if(procRes?.running && typeof procRes.retry_after === 'number'){
      ...
      await pollJobUntilDone(jobId, initialDelay);
    }catch(err){
      /* eslint-disable-next-line no-console */
      console.error(err);
  ...
  async function loadLastJobIntoView(){
    if(!lastJob?.job) return;
    const j = lastJob.job;
    if(j.result?.tracks) setTracks(j.result.tracks);
    else setMessage('Dernière génération ne contient pas de résultat.');
  }

  async function relaunchLastJob(){
    if(!lastJob?.job) return setMessage('Pas de job à relancer');
    ...
    try{
      ...
      /* eslint-disable-next-line no-console */
      console.error('relaunchLastJob error', e);
      setMessage('Erreur lors du relancement.');
    }
  }
  ...
  async function savePlaylist() {
    ...
    try{
      ...
      globalThis.location.href = '/manage';
    }catch(e){
      /* eslint-disable-next-line no-console */
      console.error(e);
  ...
  return (
    <div className="p-5">
      ...
      <div className="border border-gray-200 p-3 mb-4 rounded">
        <div>
          <label htmlFor="bpmMin">BPM min / max</label>
          <div>
            <input id="bpmMin" type="number" min="0" max="250" value={bpmMin} onChange={e=>setBpmMin(e.target.value)} className="w-20 mr-2 border rounded px-2" />
            <input id="bpmMax" type="number" min="0" max="250" value={bpmMax} onChange={e=>setBpmMax(e.target.value)} className="w-20 border rounded px-2" />
          </div>
        </div>
        ...
        <div className="mt-2">
          ...
          <div>
            <input id="danceability" type="range" min="0" max="1" step="0.01" value={danceability} onChange={e=>setDanceability(e.target.value)} className="w-full" />
          </div>
        </div>
          ...
          <div>
            {STYLES.map(s => (
              <label key={s} className="mr-2">
                <input id={`style-${s}`} type="checkbox" checked={styles.includes(s)} onChange={()=>onStyleToggle(s)} className="mr-1"/> {s}
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="mt-2">
          <label htmlFor="yearMin">Années (min / max)</label>
          <div>
            <input id="yearMin" type="number" min="1900" max={new Date().getFullYear()} value={yearMin} onChange={e=>setYearMin(e.target.value)} className="w-24 mr-2 border rounded px-2" />
            <input id="yearMax" type="number" min="1900" max={new Date().getFullYear()} value={yearMax} onChange={e=>setYearMax(e.target.value)} className="w-24 border rounded px-2" />
          </div>
        </div>
        <div className="mt-2">
          <label htmlFor="lengthMinutes">Durée (minutes)</label>
          <div>
            <input id="lengthMinutes" type="number" min="1" max="600" value={lengthMinutes} onChange={e=>setLengthMinutes(e.target.value)} className="w-24 border rounded px-2" />
          </div>
        </div>
